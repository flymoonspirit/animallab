...
mSelect.prototype = {
		...
		if (oSelect.tagName.toLowerCase() == 'select') {
			...
			if (!oSelect.disabled) {
				_select.onclick = function() {
					m_stopBubble(arguments[arguments.length - 1]);
	...
	Show : function(oObj, lGlide) {
		var _this = this;
		var oSel_List = m_MoveNode(oObj, 1);
		var aSel_List_item = oSel_List.getElementsByTagName('li');
		_this.SelectedIndex = m_MoveNode(oSel_List, 1).selectedIndex;
		_this.Hover(aSel_List_item[_this.SelectedIndex + 1]);
		var fDocClick = document.documentElement.onclick ||
		function() {
		};
...
//			oSel_List.style.top = m_SeekTp(oObj, 2) - 1 + 'px';
//			oSel_List.style.left = m_SeekTp(oObj, 3) + 'px';
			oSel_List.style.display = 'block';
			if (lGlide) {
				var nSourceHei = m_GetInnerSize(oSel_List, 1);
			...
			document.documentElement.onkeydown = function() {
				...
				switch(evt.keyCode) {
					case 13:
						...
						var fRun = document.documentElement.onclick ||
						function() {
						};
						fRun();
						break;
	...
	Selected : function(oItemObj, oSelect, nValue) {
		...
		if (oSelect.selectedIndex !== nValue) {
			...
			m_MoveNode(oItemObj.parentNode, -1).innerHTML = '<a>' + oSelect.options[nValue].text + '</a>';
		}
		return false;
	},
	Glide : function(oObj, nShei, nThei, nTime) {
		...
		if (nThei == 0) {
			nSteps /= 2;
			if (m_GetInnerSize(oObj, 1) > Math.round(nShei / nSteps)) {
				oObj.style.height = m_GetInnerSize(oObj, 1) - Math.round(nShei / nSteps) + 'px';
			...
			if (m_GetInnerSize(oObj, 1) < nThei - Math.round(nShei / nSteps)) {
				oObj.style.height = m_GetInnerSize(oObj, 1) + Math.round(nShei / nSteps) + 'px';
				_this.Timer = setTimeout(function() {
					_this.Glide(oObj, nShei, nThei, nTime);
				}, nTime);
	...
	Listen : function(oObj) {
		if (oObj.form) {
			if (oObj.form.getAttribute('mLit') !== 'yes') {
				var fFormReSet = oObj.form.onreset ||
				function() {
				};
				oObj.form.onreset = function() {
					setTimeout(function() {
						...
						for (var i = 0; i < aS.length; i++) {
							var oMSel = m_MoveNode(m_MoveNode(aS[i], -1), -1);
...
function m_GetCss(oObj, cAttrib) {
...
function m_GetInnerSize(oObj, nSize) {
	if ( nSize = 0) {
		var nResult = oObj.offsetWidth;
		if (!isNaN(parseInt(m_GetCss(oObj, 'paddingLeft'), 10))) {
			nResult -= m_GetCss(oObj, 'paddingLeft');
		}
		if (!isNaN(parseInt(m_GetCss(oObj, 'paddingRight'), 10))) {
			nResult -= m_GetCss(oObj, 'paddingRight');
		}
		if (!isNaN(parseInt(m_GetCss(oObj, 'borderLeftWidth'), 10))) {
			nResult -= m_GetCss(oObj, 'borderLeftWidth');
		}
		if (!isNaN(parseInt(m_GetCss(oObj, 'borderRightWidth'), 10))) {
			nResult -= m_GetCss(oObj, 'borderRightWidth');
		}
	} else {
		var nResult = oObj.offsetHeight;
		if (!isNaN(parseInt(m_GetCss(oObj, 'paddingTop'), 10))) {
			nResult -= m_GetCss(oObj, 'paddingTop');
		}
		if (!isNaN(parseInt(m_GetCss(oObj, 'paddingBottom'), 10))) {
			nResult -= m_GetCss(oObj, 'paddingBottom');
		}
		if (!isNaN(parseInt(m_GetCss(oObj, 'borderTopWidth'), 10))) {
			nResult -= m_GetCss(oObj, 'borderTopWidth');
		}
		if (!isNaN(parseInt(m_GetCss(oObj, 'borderBottomWidth'), 10))) {
			nResult -= m_GetCss(oObj, 'borderBottomWidth');
		}
	}
	return nResult;
}

function m_SeekTp(oObj, nDire) {
	if (oObj.getBoundingClientRect) {
		...
		if (oObj.offsetParent != null) {
			nPosition += SeekTp(oObj.offsetParent, nDire);
		}
		return nPosition;
	}
}

function m_MoveNode(oObj, nDire) {
	if (nDire < 0) {
